...
describe("toStorageData", () => {
  ...
  test("converts WorkDay to DashboardRow (decimal hours become time strings)", () => {
    const day = makeWorkDay({ startTime: 9, endTime: 18, breakStarts: [12], breakEnds: [13] });
  ...
  test("handles multiple days in order", () => {
    ...
    expect(result.rows[0].date).toBe("03/01");
    expect(result.rows[1].date).toBe("03/02");
    expect(result.rows[2].date).toBe("03/03");
  });
  ...
  test("handles fractional hours (e.g. 9.5 → 9:30)", () => {
    const day = makeWorkDay({ startTime: 9.5, endTime: 18.5 });
    const result = toStorageData([day], [], "2026-03-01T00:00:00Z");
    const row = result.rows[0];
