...
- **月カレンダー**が付く。たたんだ状態は日ごとの過不足のミニバー、開くと日ごとのセル。セルは白のまま状態を左 3px のラインで示し、過不足は中央基準の差分バー（±3:00 で振り切り）で見せる。週合計列は「▸ 週合計」で開閉する
- **日付をクリックするとその日の明細**が出る（稼働・休憩の帯 / 休憩の回数と各区間 / 実働と所定 / 申請ボタン / 表の該当行へ）。画面の端でも切れないよう、はみ出す側と反対に開く
- **KOT ページの表示は拡張が持つ**。表（28 列）・月別データ（時間集計）・ツールバー（申請・出力）は既定で隠し、拡張の表示だけを見せる。戻すときは操作行の「表示」メニューから項目ごとに切り替える（設定は保存される）
- **申請はカレンダーから**。各日のセル右下の `⋯` から KOT の申請メニュー（打刻編集 / スケジュール申請 / 時間外勤務申請）を開ける。選ぶと KOT 側のボタンをそのまま押すので、表をたたんだままでも申請できる
- **月末着地の見通し**を 0 起点の 1 本バーで出す。実働済み → これから増える分 → 着地の振れ幅（斜線）と積み上げ、所定に対する 25/50/75/100% の目盛りに％と時間を併記する。何を仮定した判定なのかも 1 行添える。詳細は[月末着地の推定](#月末着地の推定)を参照
- 絵文字を廃止し、ラベル・色・位置で階層を作る。赤は「不足」と「所定ライン」、オレンジは「打刻漏れ」だけに使う
- 最小フォントは 12px。説明文の最薄は `#5b6f75` で、`#93a5aa` は「意図的に薄い値」（未来日の推奨ペース）だけに使う
